...
export default function AboutPage() {
  return <main className="mx-auto max-w-4xl px-4 py-12 sm:px-6 lg:px-8 lg:py-16">
    <p className="text-xs font-semibold tracking-[0.18em] text-[var(--color-jade-brand)]">ABOUT THE PROJECT</p>
    <h1 className="mt-3 font-serif text-4xl font-bold text-[var(--color-forest-brand)] sm:text-5xl">让选择轻一点，也让边界说清楚。</h1>
    <div className="mt-10 space-y-7 text-base leading-8 text-stone-700">
      ...
      <section className="border-t border-stone-300 pt-7"><h2 className="font-serif text-2xl font-bold text-stone-900">北大外卖</h2><p className="mt-3">外卖商户、距离、营业时间和人均仅为公开资料或估算参考；A级表示近期官方、商场或政府资料确认，B级表示公开地图候选。距离为直线距离，部分使用商圈或楼宇中心估算。</p><p className="mt-3">本模块未接入美团或饿了么，所有内置商户均为“配送待平台确认”，不能据此判断当前是否可送、营业或在售。</p></section>
    ...
    <div className="mt-10 flex flex-wrap gap-3"><Link href="/wheel" className="rounded-full bg-[var(--color-forest-brand)] px-6 py-3 text-sm font-semibold text-white">去选一杯 →</Link><Link href="/meals" className="rounded-full bg-[var(--color-coral-deep)] px-6 py-3 text-sm font-semibold text-white">去选一顿 →</Link><Link href="/canteens" className="rounded-full bg-[var(--color-amber-highlight)] px-6 py-3 text-sm font-semibold text-[var(--color-forest-brand)]">去逛食堂 →</Link><Link href="/takeout" className="rounded-full bg-[var(--color-jade-brand)] px-6 py-3 text-sm font-semibold text-white">去抽外卖 →</Link><Link href="/data" className="rounded-full border border-stone-300 bg-white px-6 py-3 text-sm font-semibold text-stone-700">查看数据边界</Link></div>
  </main>;
}
