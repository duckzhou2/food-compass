...
import { readCssColor } from "@/lib/ui-colors";
import {
  defaultMealFilters,
  excludeLabels,
  filtersForPreset,
...
function mealWheelColor(categoryIndex: number) {
...
export function MealExperience() {
  ...
  useEffect(() => {
    ...
    if (!segments.length) {
      context.fillStyle = readCssColor("--color-canvas-empty");
      context.beginPath(); context.arc(center, center, center - 10, 0, Math.PI * 2); context.fill();
      context.fillStyle = readCssColor("--color-canvas-muted"); context.font = "700 24px Microsoft YaHei UI"; context.textAlign = "center"; context.fillText("暂无候选食物", center, center);
      return;
    }
    ...
    segments.forEach((item, index) => {
      ...
      context.fillStyle = readCssColor(mealWheelColor(categoryIndex)); context.globalAlpha = foodStage && index % 2 ? 0.76 : 1; context.fill(); context.globalAlpha = 1;
      context.strokeStyle = readCssColor("--color-wheel-line"); context.lineWidth = 2; context.stroke();
    });
    context.beginPath(); context.arc(center, center, 82, 0, Math.PI * 2); context.fillStyle = readCssColor("--color-forest-brand"); context.fill();
    ...
    <>
      ...
      <div className="grid items-start gap-6 lg:grid-cols-[310px_minmax(0,1fr)]">
        <FilterShell open={filtersOpen} onToggle={() => setFiltersOpen((value) => !value)}>
            ...
            <details className="mt-5 border-t border-stone-200 pt-4"><summary className="min-h-11 cursor-pointer py-2 text-sm font-semibold text-[var(--color-forest-brand)]">更多条件</summary>
              ...
              <label className="mt-4 flex min-h-11 items-center gap-3 text-sm"><input type="checkbox" checked={filters.onlyFavorites} onChange={(event) => changeFilters({ onlyFavorites: event.target.checked })} className="size-4 accent-[var(--color-jade-brand)]" />只看收藏</label>
              <label className="flex min-h-11 items-center gap-3 text-sm"><input type="checkbox" checked={filters.onlyCustom} onChange={(event) => changeFilters({ onlyCustom: event.target.checked })} className="size-4 accent-[var(--color-jade-brand)]" />只看自定义</label>
              <label className="flex min-h-11 items-center gap-3 text-sm"><input type="checkbox" checked={personal.settings.avoidRecent} onChange={(event) => setPersonalAndStatus({ ...personal, settings: { avoidRecent: event.target.checked } })} className="size-4 accent-[var(--color-jade-brand)]" />避免最近三次重复</label>
            </details>
        ...
        <main className="min-w-0">
          <div className="flex flex-wrap items-center justify-between gap-3">
            ...
            <button type="button" onClick={() => { setManagerEditId(null); setManagerOpen(true); }} className="min-h-11 rounded-xl border border-stone-300 bg-white px-4 text-sm font-semibold text-[var(--color-forest-brand)]">管理我的食物</button>
          </div>
          <div className="mt-4 flex flex-wrap items-center justify-between gap-3"><p className="text-sm text-stone-500">当前有 <strong className="font-mono text-lg text-[var(--color-forest-brand)]">{activeFoods.length}</strong> 个选项，覆盖 {activeCategories.length} 个分类。</p>{mode === "browse" && <label className="text-sm text-stone-500">排序 <select aria-label="食物排序" value={filters.sort} onChange={(event) => changeFilters({ sort: event.target.value as MealFilters["sort"] })} className="ml-2 min-h-11 rounded-xl border border-stone-300 bg-white px-3 text-stone-700"><option value="source">推荐顺序</option><option value="name">名称</option><option value="price">价格由低到高</option><option value="time">用餐时间由快到慢</option><option value="recent">最近添加</option></select></label>}</div>

          {mode === "browse" ? (
            <div className="mt-5">
              {result && <div className="mb-6">{result}</div>}
              {browseFoods.length ? <><section aria-label="食物列表" className="grid gap-3 sm:grid-cols-2">{browseFoods.slice(0, visibleCount).map((food) => <button key={food.id} type="button" onClick={() => reveal(food)} className={`min-w-0 rounded-3xl border bg-white p-5 text-left transition hover:-translate-y-0.5 hover:border-[var(--color-jade-brand)] ${selected?.id === food.id ? "border-[var(--color-jade-brand)] shadow-md" : "border-stone-200"}`}><span className="text-xs font-semibold tracking-[0.12em] text-[var(--color-jade-brand)]">{getMealCategory(food.categoryId).name}{food.source === "custom" ? " · 自定义" : ""}</span><span className="mt-2 block break-words font-serif text-xl font-bold text-stone-900">{food.name}</span><span className="mt-3 block text-xs leading-5 text-stone-500">{formatMealPrice(food.priceBands)} · {food.timeTags.map((tag) => timeLabels[tag]).join(" / ") || "时间未填写"}</span></button>)}</section>{visibleCount < browseFoods.length && <button type="button" onClick={() => setVisibleCount((count) => count + 24)} className="mt-5 min-h-11 w-full rounded-xl border border-stone-300 bg-white text-sm font-semibold text-stone-700">加载更多</button>}</> : <EmptyMealState onReset={() => changeFilters({ ...defaultMealFilters })} />}
            </div>
          ) : (
            <div className="mt-5">
              {recentPool.foods.length ? <section className="overflow-hidden rounded-[2rem] bg-[var(--color-surface-panel)] px-4 py-7 text-center sm:px-8"><p className="text-xs font-semibold tracking-[0.16em] text-[var(--color-jade-brand)]">{foodStage ? "具体食物转盘" : "第一步 · 选择大类"}</p><h2 className="mt-2 font-serif text-2xl font-bold text-[var(--color-forest-brand)]">{lockedCategoryId && !manualCategorySelection ? `从${getMealCategory(lockedCategoryId).name}中选一个` : foodStage ? "从所选分类中直接选一个" : "先决定今天吃哪一类"}</h2>{lockedCategoryId && !manualCategorySelection && <button type="button" onClick={() => { setLockedCategoryId(null); setProposedCategoryId(null); setSelected(null); }} className="mt-3 min-h-11 rounded-xl border border-stone-300 bg-white px-4 text-sm font-semibold text-[var(--color-forest-brand)]">返回重选大类</button>}<div className="relative mx-auto mt-6 aspect-square w-full max-w-[540px]"><div className="absolute left-1/2 top-0 z-20 -translate-x-1/2 border-x-[16px] border-t-[30px] border-x-transparent border-t-[var(--color-coral-deep)] drop-shadow" /><canvas ref={wheelRef} width="600" height="600" className="size-full rounded-full border-[9px] border-white bg-white shadow-[var(--shadow-wheel)]" style={{ transform: `rotate(${rotation}deg)`, transition: reducedMotion ? "transform 80ms linear" : "transform var(--dur-wheel) var(--ease-out)" }} /><button type="button" onClick={spin} disabled={isSpinning} className="absolute left-1/2 top-1/2 z-30 grid size-24 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full border-[8px] border-white bg-[var(--color-forest-brand)] font-serif text-lg font-bold text-white shadow-xl disabled:opacity-80 sm:size-28">{isSpinning ? "转动中" : foodStage ? "转一道" : proposedCategoryId ? "换一类" : "转大类"}</button></div><p className="mt-5 text-xs leading-5 text-stone-500">{foodStage ? `盘面展示 ${wheelFoods.length} 个代表选项，最终结果从全部 ${foodStagePool.length} 个候选中产生。` : "先抽大类并确认，再进入该分类的具体食物转盘。"}</p></section> : <EmptyMealState onReset={() => changeFilters({ ...defaultMealFilters })} />}
              {proposedCategoryId && !foodStage && <section className="mt-6 rounded-3xl border border-amber-200 bg-amber-50 px-5 py-6 text-center" aria-live="polite"><p className="text-sm text-amber-900">第一步抽中了</p><h3 className="mt-2 font-serif text-3xl font-bold text-[var(--color-forest-brand)]">{getMealCategory(proposedCategoryId).name}</h3><p className="mt-2 text-sm text-stone-600">要从这个大类里继续选一道吗？</p><div className="mx-auto mt-5 grid max-w-md gap-2 sm:grid-cols-2"><button type="button" onClick={() => { setLockedCategoryId(proposedCategoryId); setProposedCategoryId(null); setSelected(null); }} className="min-h-12 rounded-xl bg-[var(--color-forest-brand)] px-4 font-semibold text-white">就选这个大类</button><button type="button" onClick={spin} className="min-h-12 rounded-xl border border-stone-300 bg-white px-4 font-semibold text-stone-700">换个大类</button></div></section>}
...
function EmptyMealState({ onReset }: { onReset: () => void }) { return <div className="mt-5 rounded-3xl border border-dashed border-stone-300 bg-white/60 px-6 py-16 text-center"><p className="font-serif text-2xl font-bold text-stone-800">没有符合全部条件的选项</p><p className="mt-3 text-sm leading-6 text-stone-500">建议放宽预算、用餐时间或排除条件。</p><button type="button" onClick={onReset} className="mt-5 min-h-11 rounded-xl bg-[var(--color-forest-brand)] px-5 text-sm font-semibold text-white">放宽全部条件</button></div>; }
