    ...
    <div role="dialog" aria-modal="true" aria-labelledby="takeout-manager-title" className="fixed inset-0 z-[90] flex justify-end bg-black/45">
      <div className="h-full w-full max-w-3xl overflow-auto bg-[#f8f5ee] p-5 sm:p-8">
        ...
        <div className="mt-6 flex gap-2 overflow-x-auto rounded-2xl bg-stone-200/70 p-1">
          {([['custom','自定义商户'],['favorites','收藏'],['history','最近点过'],['excluded','永久排除'],['transfer','导入导出']] as const).map(([id, label]) => <button key={id} type="button" onClick={() => setTab(id)} className={`min-h-11 whitespace-nowrap rounded-xl px-3 text-sm font-semibold ${tab === id ? "bg-white text-[#173f35] shadow-sm" : "text-stone-500"}`}>{label}</button>)}
        </div>
...
function CustomMerchantForm({ editing, categories, areas, onSave, onCancel }: { editing: TakeoutMerchant | null; categories: TakeoutReferenceItem[]; areas: TakeoutReferenceItem[]; onSave: (merchant: TakeoutMerchant) => void; onCancel: () => void }) {
  ...
  return <form key={key} onSubmit={(event) => {
    ...
    <div className="flex gap-2 sm:col-span-2"><button type="submit" className="min-h-11 rounded-xl bg-[#173f35] px-5 font-semibold text-white">{editing ? "保存修改" : "添加商户"}</button>{editing && <button type="button" onClick={onCancel} className="min-h-11 rounded-xl border border-stone-300 px-4">取消编辑</button>}</div>
  </form>;
}
