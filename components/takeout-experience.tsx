...
export function TakeoutExperience() {
  ...
  useEffect(() => {
    ...
    if (!segments.length) { context.beginPath(); context.arc(center, center, center - 8, 0, Math.PI * 2); context.fillStyle = readCssColor("--color-canvas-empty"); context.fill(); context.fillStyle = readCssColor("--color-canvas-muted"); context.font = "700 22px Microsoft YaHei UI"; context.textAlign = "center"; context.fillText("暂无候选商户", center, center); return; }
    ...
    segments.forEach((item, index) => {
      ...
      context.fillStyle = readCssColor(palette[(categoryIndex + index) % palette.length]); context.fill();
      context.strokeStyle = readCssColor("--color-wheel-line"); context.lineWidth = 2; context.stroke();
    });
    context.beginPath(); context.arc(center, center, 78, 0, Math.PI * 2); context.fillStyle = readCssColor("--color-forest-brand"); context.fill();
  ...
  const spin = () => {
    ...
    window.setTimeout(() => { reveal(picked.merchant, recentPool.relaxed ? "为保留候选，已适度放宽最近三次防重复。" : ""); setSpinning(false); }, reducedMotion ? 80 : 2400);
  };
  ...
  return <>
    ...
    <div className="grid items-start gap-6 lg:grid-cols-[310px_minmax(0,1fr)]">
      <FilterShell open={filtersOpen} onToggle={() => setFiltersOpen((value) => !value)}>
          ...
          <details className="mt-5 border-t border-stone-200 pt-4"><summary className="min-h-11 cursor-pointer py-2 text-sm font-semibold text-[var(--color-forest-brand)]">更多条件</summary><FilterSection title="信息可信度"><PillGroup items={evidenceLevels} selected={filters.evidenceLevels} onChange={(values) => changeFilters({ evidenceLevels: values as ("A" | "B")[] })} /></FilterSection><label className="mt-3 flex min-h-11 items-center gap-3 text-sm"><input type="checkbox" checked={filters.onlyFavorites} onChange={(event) => changeFilters({ onlyFavorites: event.target.checked })} className="size-4 accent-[var(--color-jade-brand)]" />只看收藏</label><label className="flex min-h-11 items-center gap-3 text-sm"><input type="checkbox" checked={personal.settings.avoidRecent} onChange={(event) => persist({ ...personal, settings: { avoidRecent: event.target.checked } })} className="size-4 accent-[var(--color-jade-brand)]" />避免最近三次重复</label><label className="flex min-h-11 items-center gap-3 text-sm"><input type="checkbox" checked={filters.excludeEstimatedDistance} onChange={(event) => changeFilters({ excludeEstimatedDistance: event.target.checked })} className="size-4 accent-[var(--color-jade-brand)]" />排除商圈中心估算距离</label></details>
...
function EmptyState({ onReset }: { onReset: () => void }) { return <div className="rounded-[1.75rem] border border-dashed border-stone-300 bg-white/60 p-10 text-center"><h3 className="font-serif text-2xl font-bold text-[var(--color-forest-brand)]">没有符合条件的商户</h3><p className="mt-2 text-sm text-stone-500">可以放宽类型、商圈或距离条件后再试。</p><button type="button" onClick={onReset} className="mt-5 min-h-11 rounded-xl bg-[var(--color-forest-brand)] px-5 font-semibold text-white">清空全部条件</button></div>; }
