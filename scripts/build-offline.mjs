...
import { loadOfflineResources } from "./offline-resources.mjs";
...
const { siteContent, assets } = await loadOfflineResources(projectRoot);
const moduleById = Object.fromEntries(siteContent.modules.map((module) => [module.id, module]));
...
const moduleNavigation = siteContent.modules
  .map((module) => `<a href="#${sectionIdByModule[module.id]}">${module.label}</a>`)
  .join("");
const moduleCards = siteContent.modules
  .map((module) => `
    <a class="home-module-card home-module-${module.tone}" href="#${sectionIdByModule[module.id]}">
      <img src="${assets[module.id]}" alt="" width="800" height="600">
...
const uiTokens = await readFile(path.join(projectRoot, "tokens.css"), "utf8");
...
<head>
  ...
  <style>
${uiTokens}
    :root{--paper:var(--color-paper-alt);--ink:var(--color-ink);--forest:var(--color-forest-brand);--jade:var(--color-jade-brand);--amber:var(--color-amber-highlight);--coral:var(--color-coral-deep);--line:var(--color-border-strong);--muted:var(--color-canvas-muted);--white:oklch(100% 0 0);--soft:var(--color-surface-filter);--shadow:var(--shadow-result)}
    *{box-sizing:border-box}html{scroll-behavior:smooth}body{margin:0;overflow-x:hidden;background:radial-gradient(circle at 8% 3%,rgba(232,165,75,.11),transparent 24rem),radial-gradient(circle at 94% 16%,rgba(23,107,85,.09),transparent 27rem),var(--paper);color:var(--ink);font-family:"Microsoft YaHei UI","PingFang SC",system-ui,sans-serif}button,input,select,summary{font:inherit}button,a,input,select,summary{-webkit-tap-highlight-color:transparent}button{cursor:pointer}button:disabled{cursor:not-allowed}.shell{width:min(1200px,calc(100% - 32px));margin:auto}.hidden{display:none!important}
    .topbar{position:sticky;top:0;z-index:50;padding:8px 8px 0;background:color-mix(in oklch,var(--paper) 80%,transparent)}.topbar-inner{display:flex;align-items:center;gap:18px;min-height:70px;padding:0 16px;border:1px solid color-mix(in oklch,var(--line) 75%,transparent);border-radius:24px;background:var(--paper);box-shadow:var(--shadow-panel)}.brand{display:flex;align-items:center;gap:10px;color:var(--forest);font-family:Georgia,"Songti SC",serif;font-weight:800;text-decoration:none;white-space:nowrap}.brand-mark{display:grid;place-items:center;width:36px;height:36px;border-radius:50%;background:var(--forest);color:#f6cf72}.nav{display:flex;align-items:center;gap:4px;margin-left:auto}.nav a{padding:9px 12px;border-radius:999px;color:#67635d;font-size:14px;text-decoration:none}.nav a:hover,.nav a.active{background:#e9e2d4;color:var(--forest)}.offline-badge{padding:6px 9px;border:1px solid #bed5cb;border-radius:999px;color:var(--jade);font-size:11px;font-weight:800;white-space:nowrap}
    .page{display:none;min-height:calc(100vh - 142px);padding:54px 0 72px}.page.active{display:block}.eyebrow{margin:0;color:var(--jade);font-size:12px;font-weight:800;letter-spacing:.16em}.page-title,.hero h1{color:var(--forest);font-family:Georgia,"Songti SC",serif;font-weight:800;letter-spacing:-.035em}.page-title{margin:12px 0 0;font-size:clamp(38px,5vw,58px);line-height:1.12}.lead{max-width:760px;margin:20px 0 0;color:#625f59;font-size:16px;line-height:1.9}.hero{display:grid;grid-template-columns:1.02fr .98fr;align-items:center;gap:42px;min-height:610px}.hero h1{margin:22px 0 0;font-size:clamp(48px,7vw,80px);line-height:1.03}.hero h1 span{display:block;margin-top:8px;color:var(--coral)}.actions{display:flex;flex-wrap:wrap;gap:12px;margin-top:30px}.primary,.secondary{display:inline-flex;align-items:center;justify-content:center;min-height:46px;padding:0 22px;border-radius:999px;font-size:14px;font-weight:800;text-decoration:none}.primary{border:0;background:var(--forest);color:#fff;box-shadow:0 12px 30px rgba(23,63,53,.2)}.secondary{border:1px solid var(--line);background:#fff;color:#4f4b45}.hero-visual{position:relative;overflow:hidden;min-height:460px;border-radius:40px;background:var(--forest);box-shadow:0 35px 80px rgba(23,63,53,.2)}.hero-visual img{position:absolute;inset:0;width:100%;height:100%;object-fit:cover}.hero-visual::after{content:"";position:absolute;inset:45% 0 0;background:linear-gradient(transparent,rgba(23,63,53,.72))}.hero-visual-note{position:absolute;z-index:1;right:24px;bottom:22px;left:24px;color:#fff;font:800 21px Georgia,"Songti SC",serif}.home-module-grid{display:grid;grid-template-columns:repeat(4,minmax(0,1fr));gap:14px;margin-top:10px}.home-module-card{display:block;overflow:hidden;border:1px solid var(--line);border-radius:26px;background:#fff;color:var(--forest);text-decoration:none;transition:.18s}.home-module-card:hover{transform:translateY(-3px);box-shadow:0 16px 40px rgba(50,45,35,.1)}.home-module-card img{display:block;width:100%;aspect-ratio:4/3;object-fit:cover}.home-module-copy{display:block;padding:17px}.home-module-copy small,.home-module-copy strong,.home-module-copy span{display:block}.home-module-copy small{color:var(--jade);font-size:9px;font-weight:800;letter-spacing:.13em}.home-module-copy strong{margin-top:7px;font:800 21px Georgia,"Songti SC",serif}.home-module-copy span{margin-top:10px;color:var(--coral);font-size:12px;font-weight:800}
...
${takeoutCss}
${milkPersonalCss}
    button,a{transition-property:color,background-color,border-color,box-shadow,opacity,transform;transition-duration:var(--dur-short);transition-timing-function:var(--ease-out)}
    @media (prefers-reduced-motion: reduce){*,*::before,*::after{scroll-behavior:auto!important;animation-duration:.01ms!important;animation-iteration-count:1!important}.wheel-canvas,.canteen-wheel-disc{transition-duration:80ms!important;transition-timing-function:linear!important}}
    @media(max-width:900px){.hero{grid-template-columns:1fr;min-height:auto}.hero-visual{min-height:400px}.home-module-grid{grid-template-columns:repeat(2,minmax(0,1fr))}.workspace{grid-template-columns:1fr}.filters{position:static}.filter-summary{display:block;cursor:pointer;color:var(--forest);font-weight:800}.filters:not([open]) .filter-head,.filters:not([open]) .filter-body{display:none}.product-grid{grid-template-columns:repeat(2,minmax(0,1fr))}}
    @media(max-width:640px){.shell{width:min(100% - 22px,1200px)}.topbar-inner{min-height:62px;gap:8px}.brand span:last-child{display:none}.nav{max-width:calc(100vw - 112px);overflow-x:auto;gap:0}.nav a{padding:8px;font-size:12px}.offline-badge{display:none}.page{padding:36px 0 54px}.hero{gap:28px}.hero h1{font-size:44px}.hero-visual{min-height:320px;border-radius:28px}.home-module-grid{gap:10px}.home-module-card{border-radius:20px}.home-module-copy{padding:13px}.home-module-copy strong{font-size:17px}.page-head{margin-bottom:22px}.product-grid,.brand-data-grid,.topping-grid{grid-template-columns:1fr}.toolbar{align-items:flex-start;flex-direction:column}.wheel-card{padding:22px 10px}.spin{width:88px;height:88px;border-width:7px;font-size:15px}.result-head,.result-body{padding:22px 18px}.result-summary{grid-template-columns:1fr}.calorie-box{grid-template-columns:1fr}.stats{grid-template-columns:1fr;padding:0}.stat{border-right:0;border-bottom:1px solid var(--line);padding:18px 0}.stat:last-child{border-bottom:0}.brand-data-card dl{grid-template-columns:repeat(3,minmax(0,1fr))}.footer-inner{flex-direction:column}.choice-pills{max-height:160px;overflow:auto}}
...
<body>
  <header class="topbar">
    <div class="shell topbar-inner">
      ...
      <nav class="nav" aria-label="主导航"><a href="#home">首页</a>${moduleNavigation}<a href="#data">数据</a><a href="#about">说明</a></nav>
  ...
  <main>
    <section class="page" id="home">
      <div class="shell">
        <div class="hero">
          <div><p class="eyebrow">${siteContent.brand.englishName}</p><h1>今天吃什么？<span>让罗盘替你选。</span></h1><p class="lead">从一杯奶茶到一顿正餐，从北大食堂到北大外卖。按条件筛一下，或者直接把决定交给罗盘。</p><div class="actions"><a class="primary" href="#wheel">先选一杯</a><a class="secondary" href="#food">开始选一顿</a></div></div>
          <div class="hero-visual"><img src="${assets.hero}" alt="" width="1600" height="900"><span class="hero-visual-note">四套选择工具 · 数据与偏好全部留在本地</span></div>
        </div>
        <div class="home-module-grid">${moduleCards}</div>
        <b id="heroProductCount" style="display:none">0</b><b id="heroVariantCount" style="display:none">0</b><b id="heroToppingCount" style="display:none">0</b>
      </div>
    </section>
...
${takeoutMarkup}
    ...
    <section class="page" id="data"><div class="shell"><div class="page-head"><p class="eyebrow">DATA OVERVIEW</p><h1 class="page-title">四套选择，一处说明数据边界</h1><p class="lead">奶茶、正餐、北大食堂和北大外卖全部静态写入本文件，并使用彼此独立的本地存储。</p></div><div class="mode-switch mode-tabs" style="max-width:720px"><button class="active" data-offline-data="milk" type="button">奶茶数据</button><button data-offline-data="meals" type="button">食物选项</button><button data-offline-data="canteens" type="button">北大食堂</button><button data-offline-data="takeout" type="button">北大外卖</button></div><section class="hidden" id="offlineMealData"><div class="stats"><div class="stat"><div class="stat-label">内置食物</div><div class="stat-value">459</div></div><div class="stat"><div class="stat-label">主分类</div><div class="stat-value">9</div></div><div class="stat"><div class="stat-label">自定义</div><div class="stat-value" id="offlineCustomCount">0</div></div></div><div class="brand-data-grid" id="offlineMealCategoryGrid"></div><p class="prose">价格、口味、时间和饱腹标签是一般化决策参考，不是官方门店数据。</p></section><section class="hidden" id="offlineCanteenData"><div class="stats"><div class="stat"><div class="stat-label">正式食堂</div><div class="stat-value">14</div></div><div class="stat"><div class="stat-label">窗口/档口</div><div class="stat-value">79</div></div><div class="stat"><div class="stat-label">结构化菜品</div><div class="stat-value">650</div></div><div class="stat"><div class="stat-label">历史SKU</div><div class="stat-value">514</div></div></div><p class="prose">窗口待确认菜品427个；历史价格与营养不代表当前在售或医学精确测量。</p></section><section class="hidden" id="offlineTakeoutData"><div class="stats"><div class="stat"><div class="stat-label">候选商户</div><div class="stat-value">339</div></div><div class="stat"><div class="stat-label">主分类</div><div class="stat-value">9</div></div><div class="stat"><div class="stat-label">覆盖商圈</div><div class="stat-value">9</div></div><div class="stat"><div class="stat-label">A级证据</div><div class="stat-value">73</div></div></div><div class="brand-data-grid">${takeoutCatalog.referenceData.categories.map((category) => `<article class="brand-data-card"><h2>${category.name}</h2><dl><div><dt>候选商户</dt><dd>${category.merchantCount}</dd></div></dl></article>`).join("")}</div><p class="prose" id="offlineTakeoutPersonalStats">公开候选库未接入美团或饿了么，不能据此判断当前是否可送。</p></section><section id="offlineMilkData"><div class="stats"><div class="stat"><div class="stat-label">品牌</div><div class="stat-value" id="brandCount">0</div></div><div class="stat"><div class="stat-label">独立产品</div><div class="stat-value" id="productCount">0</div></div><div class="stat"><div class="stat-label">规格记录</div><div class="stat-value" id="variantCount">0</div></div></div><div class="brand-data-grid" id="brandDataGrid"></div><p class="prose" id="milkTeaPersonalStats"></p></section></div></section>

    <section class="page" id="about"><div class="shell about-wrap"><p class="eyebrow">ABOUT</p><h1 class="page-title">让选择轻一点，也把边界说清楚</h1><p class="prose">正餐标签为一般化参考；北大食堂资料严格区分当前官方、官方历史、GitHub历史、推断与待核验；奶茶缺失规格不推算、不平均。</p><p class="prose"><strong>${moduleById.takeout.label}：</strong>商户、距离、营业时间和人均仅为公开资料或估算参考。未接入美团或饿了么，所有内置商户均为“配送待平台确认”，不能据此判断当前是否可送、营业或在售。</p><p class="prose">食堂历史价格和每份营养不代表当前在售、现价或官方精确测量。所有收藏、排除与历史只保存在当前浏览器本地。</p><div class="notice">本网站不提供定位、实时菜单、实时价格、过敏原、医疗、减重或专业营养建议。</div></div></section>
  </main>

  <footer class="footer"><div class="shell footer-inner"><span>食物罗盘 · 单文件离线版</span><span>奶茶、正餐、北大食堂与北大外卖数据静态内嵌，无需联网</span></div></footer>
${milkPersonalMarkup}
  ...
  <script>
    ...
    var COLORS = ["--color-jade-brand","--color-amber-highlight","--color-coral-deep","--color-wheel-blue","--color-wheel-tan","--color-wheel-sage","--color-wheel-rose","--color-wheel-berry"];function cssColor(token){return getComputedStyle(document.documentElement).getPropertyValue(token).trim()||"transparent";}
    ...
    function drawWheel(){var canvas=byId("wheelCanvas");var context=canvas.getContext("2d");var size=canvas.width;var center=size/2;context.clearRect(0,0,size,size);var products=wheelProducts();if(!products.length){context.fillStyle=cssColor("--color-canvas-empty");context.beginPath();context.arc(center,center,center-8,0,Math.PI*2);context.fill();context.fillStyle=cssColor("--color-canvas-muted");context.font="700 22px Microsoft YaHei UI";context.textAlign="center";context.fillText("暂无候选饮品",center,center);byId("spinButton").disabled=true;return;}byId("spinButton").disabled=state.spinning;var count=Math.min(products.length,12);var segment=Math.PI*2/count;for(var index=0;index<count;index+=1){context.beginPath();context.moveTo(center,center);context.arc(center,center,center-8,-Math.PI/2+index*segment,-Math.PI/2+(index+1)*segment);context.closePath();context.fillStyle=cssColor(COLORS[index%COLORS.length]);context.fill();}context.beginPath();context.arc(center,center,82,0,Math.PI*2);context.fillStyle=cssColor("--color-forest-brand");context.fill();}
